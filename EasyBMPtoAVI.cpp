	...
	**************************************************************/
	...
	const int GlobalSize = 50;

	int ArrayofInts[GlobalSize];
	//int MinofArray = 901;
	//int MaxofArray;
	const int TopofBar = 0;
	const int BarHeight = 4;
	const int LeftofBar = 0;
	const int spacebetweenbars = 1;

	// draws the from..to portion of the index'th bar in colorvalue into BMP img
	void MakeBar(int from, int to, BMP & img, int index, int ColorValue){
		for ( from += LeftofBar; from < (to + LeftofBar); from++)
			// fill in bar from top to bottom of bar

			// iterate over columns that form bar or something like that
			for (int pixelColumn = 0; pixelColumn < BarHeight; pixelColumn++)
				img(from, pixelColumn + TopofBar + (spacebetweenbars + BarHeight) * index)->Red = ColorValue;

	}

	int iFrame = 0;

	void WriteOutFrame(BMP & img){
		char strFileName[100];
		sprintf(strFileName, "C:\\Windows\\Temp\\FramesforCS315\\Frame%04d.bmp", iFrame++);
		img.WriteToFile(strFileName);
	}

	void startRandImage(BMP & img, int k){
		// loop that will first generate random value and then draw corresponding bar to image
		...
		for (int i = 0; i < k; i++){
			...
			ArrayofInts[i] = temp;
			//	Bars.push_back(temp);
			// fill in bar from left to right within bar
			MakeBar(0, ArrayofInts[i], img, i, 255);
		}
	...
	void Update(BMP & img, int index, int value){

		// find difference between value passed in and value at index
		signed int Difference = ArrayofInts[index] - value;
		int TopofCurrentBar = (spacebetweenbars + BarHeight) * value - 1;

		// if difference is negative then fill to passed in value on new image
		if (Difference < 0)
			// set bar from 0 to value to red
			MakeBar(ArrayofInts[index], value, img, index, 255);

		// if difference is positive then set white until at difference on new image
		// set bar from index to value as white

		else MakeBar(value, ArrayofInts[index], img, index, 0);
	...
	void shiftRight(int* arr, int low, int high, BMP & img)
	{
		int root = low;
		while ((root * 2) + 1 <= high)
		{
			...
			if (swapIdx != root)
			{
				unsigned int tmp = arr[root];
				Update(img, root, arr[swapIdx]);
				arr[root] = arr[swapIdx];
				Update(img, swapIdx, tmp);
				arr[swapIdx] = tmp;
	...
	void heapify(int* arr, int low, int high, BMP & img)
	{
		...
		while (midIdx >= 0)
		{
			...
			shiftRight(arr, midIdx, high, img);
			--midIdx;
		}
		return;
	}

	void heapSort(int* arr, int size, BMP & img)
	{
		assert(arr);
		assert(size > 0);
	...
	void PointtoBlack(BMP & myImage, Point p) {
		myImage(p.x, p.y)->Blue = 0; // function to set point to red
		myImage(p.x, p.y)->Green = 0; // parameters of referenced image and p of struct type p
		myImage(p.x, p.y)->Red = 0; // changes pixel element RGB so that only red is present in pixel
	}
	...
	int main(int argc, char* argv[])
	{
		...
		Point p;
		Image.SetSize(650, 1010);
		for (int pixelrow = 0; pixelrow < 650; pixelrow++){
			for (int pixelcolumn = 0; pixelcolumn < 1010; pixelcolumn++){
				p.x = pixelrow;
				p.y = pixelcolumn;
				PointtoBlack(Image, p);
			}
		}
		// put frame functions within heap functions!!
		startRandImage(Image, GlobalSize);

		heapSort(ArrayofInts, GlobalSize, Image);
